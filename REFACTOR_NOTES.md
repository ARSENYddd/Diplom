...
## Phase 1.5b — Верификация на нетривиальном тикере (^VIX)

**Цель:** убедиться, что Phase 1.5 имеет (или не имеет) измеримый эффект на тикере
с нетривиальным порядком ARIMA.

### Выбор тикера

| Тикер | ARIMA order | Выбран |
|---|---|---|
| BTC-USD | (0,1,0) | нет |
| ^VIX | **(1,1,1)** | **да** |
| EURUSD=X | (0,1,0) | нет |

### Ключевое открытие: auto_arima детерминирован

Три независимых вызова `auto_arima` на одних данных (`train_core`, одинаковые
`max_p`, `max_q`, `d`, `information_criterion`, `stepwise`) дают **абсолютно
идентичные объекты** — `max_abs_diff = 0.00e+00` на всех val-шагах.

Это означает: `val_preds["arima"][i]` ≡ `hybrid_own_arima_fc[i]` ≡ `triple_own_arima_fc[i]`

Следствие: Phase 1.5 исправляет **логическую некорректность**, но при текущих
гиперпараметрах (`stepwise=True`, детерминированная инициализация pmdarima)
**численный эффект равен нулю**.

### Сравнение метрик (^VIX, 3 запуска × 2 версии)

| | MAE mean±std | RMSE mean±std |
|---|---|---|
| Pre-fix (старая логика) | 0.6700 ± 0.0082 | 0.8800 ± 0.0000 |
| Post-fix (Phase 1.5) | 0.6700 ± 0.0000 | 0.8833 ± 0.0047 |
| Δ | 0.0000 | 0.0033 |
| Шум (TF-стохастика) | 0.0082 | 0.0047 |
| Эффект виден? | **НЕТ** (Δ < шум) | **НЕТ** (Δ < шум) |

### Вывод

Phase 1.5 — корректный логический фикс. Практический эффект равен нулю потому,
что `auto_arima` детерминирован: "чужие" и "свои" остатки за val всегда идентичны.

Единственный сценарий, где фикс имел бы численный эффект — если бы внутри
`_fit_and_predict_hybrid` использовался другой алгоритм подбора порядка
(например, случайный поиск, или если бы pmdarima вернул разные результаты при
параллельном запуске). В текущей кодовой базе этого нет.

**Итог:** фикс оставляем (код стал логически согласованным), числа не изменились.
