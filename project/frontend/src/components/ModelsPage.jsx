...
Это самая комплексная одиночная модель в платформе — фактически ансамбль внутри одной архитектуры.`,
  ...
  },
  {
    ...
    description: `Ансамблевая модель запускает все 15 моделей и строит взвешенное среднее их предсказаний. Веса подбираются по обратной ошибке на валидационной выборке: более точная модель получает больший вес.
...
Ensemble — это "страховой" прогноз: даже если одна модель ошибается в конкретной ситуации, другие её компенсируют.`,
    ...
    weaknesses: [
      'Самый медленный — запускает все 15 моделей',
  ...
  },
  {
    ...
    speedColor: 'text-green-400',
    complexity: 'Базовая',
    tags: ['Сезонность', 'Статистика', 'Классика'],
    description: `SARIMA — расширение классической ARIMA, добавляющее явное моделирование сезонности. Обозначается SARIMA(p,d,q)(P,D,Q)[m], где m — сезонный период (например, 5 для торговых дней, 52 для недель).

Сезонные параметры (P, D, Q) работают аналогично несезонным, но на лагах кратных m. Это позволяет захватывать повторяющиеся годовые, квартальные или недельные паттерны, которые обычная ARIMA игнорирует.

В AlphaSignal параметры подбираются автоматически через pmdarima (auto_arima с seasonal=True). Сезонный период определяется из выбранного интервала данных.`,
    strengths: [
      'Явно моделирует сезонные паттерны',
      'Автоматический подбор параметров',
      'Лучше ARIMA на данных с сезонностью',
      'Быстрый расчёт — секунды',
    ],
    weaknesses: [
      'Только линейные зависимости',
      'На данных без сезонности не лучше ARIMA',
      'Сложнее интерпретировать чем чистая ARIMA',
    ],
    bestFor: 'Активы с выраженной сезонностью: товарные фьючерсы, газ, акции с квартальными циклами.',
    notFor: 'Акции без сезонных паттернов. Инфраструктура с очень высокой волатильностью.',
    assets: ['Газ (NG=F)', 'Brent', 'Новатэк', 'Газпром', 'S&P 500 (недельный)'],
    refs: [
      {
  ...
  },
  {
    ...
    icon: <IconWave size={22}/>,
    name: 'Prophet',
    full: 'Prophet (Meta, 2017)',
    color: '#fb923c',
    mape: '1.28%',
    speed: 'Быстро',
    speedColor: 'text-green-400',
    complexity: 'Средняя',
    tags: ['Сезонность', 'Meta', 'Тренд'],
    description: `Prophet — аддитивная модель временных рядов, разработанная командой Meta (Facebook) в 2017 году. Разложение сигнала на три компонента: тренд (кусочно-линейный или логистический), сезонность (годовая, недельная, дневная через ряды Фурье) и эффекты праздников.

Ключевое преимущество — устойчивость к пропускам данных, аномалиям и сдвигам тренда. Changepoint detection автоматически находит точки изменения тренда.

Особенно эффективен когда в данных есть чёткие сезонные паттерны — январский эффект, отчётные периоды, сезонные пики спроса на сырьё.`,
    strengths: [
      'Автоматическое выделение тренда и сезонности',
      'Устойчив к пропускам данных и выбросам',
      'Интерпретируемые компоненты (можно посмотреть тренд отдельно)',
      'Не требует нормализации и дифференцирования',
    ],
    weaknesses: [
      'Слабее нейросетей на сложных нелинейных паттернах',
      'Оптимален только при наличии сезонности',
      'Менее точен на короткосрочных горизонтах',
    ],
    bestFor: 'Длинные ряды с выраженной сезонностью: товарные фьючерсы, ETF, фондовые индексы.',
    notFor: 'Высокочастотные данные без сезонности. Внутридневная торговля.',
    assets: ['Brent', 'Золото (GC=F)', 'S&P 500', 'Газ (NG=F)', 'Магнит'],
    refs: [
      {
  ...
  },
  {
    ...
    speedColor: 'text-green-400',
    complexity: 'Средняя',
    tags: ['Деревья', 'ML', 'Признаки'],
    description: `XGBoost — ансамбль деревьев решений с градиентным бустингом, разработанный Ченом и Гестрином в 2016 году. Каждое новое дерево обучается исправлять ошибки предыдущих, что даёт исключительную точность.

Для временных рядов строятся lag-признаки (прошлые цены на 1, 2, 3, 5, 10, 20, 60 периодов назад) и скользящие статистики (mean, std за 5, 10, 20, 60 периодов). Модель walk-forward — прогноз строится без перефитинга на новых данных.

Выгодно отличается от нейросетей: не требует нормализации данных, быстро обучается, есть встроенная feature importance для интерпретации.`,
    strengths: [
      'Не требует нормализации данных',
      'Быстрее нейросетей в 5–10 раз',
      'Feature importance — понятно что важно',
      'Устойчив к выбросам',
    ],
    weaknesses: [
      'Требует ручного конструирования признаков',
      'Не улавливает очень долгосрочные зависимости',
      'Хуже LSTM при смене рыночных режимов',
    ],
    bestFor: 'Задачи где важна интерпретируемость. Рынки с чёткими лаговыми зависимостями.',
    notFor: 'Ряды без чётких лаговых паттернов. Очень долгосрочные прогнозы.',
    assets: ['Сбербанк', 'Лукойл', 'Apple', 'Microsoft', 'S&P 500'],
    refs: [
      {
  ...
  },
  {
    ...
    speedColor: 'text-amber-400',
    complexity: 'Высокая',
    tags: ['Свёртки', 'Нейросеть', 'Параллельность'],
    description: `TCN — нейросетевая архитектура на основе причинных дилатированных свёрток, предложенная Байом и соавторами в 2018 году. В отличие от LSTM, полностью параллельна при обучении, что значительно ускоряет тренировку.

Дилатированные свёртки (коэффициенты 1, 2, 4, 8) экспоненциально расширяют рецептивное поле — модель может охватить 120+ временных шагов при небольшой глубине сети. Residual-соединения предотвращают затухание градиентов.

В AlphaSignal: 4 блока с дилатацией 1/2/4/8, 64 фильтра, kernel_size=3. Остаточное подключение через 1×1 свёртку выравнивает размерности.`,
    strengths: [
      'Параллельное обучение — быстрее LSTM',
      'Большое рецептивное поле без глубокой рекуррентности',
      'Residual-соединения — стабильное обучение',
      'Лучше масштабируется на длинных последовательностях',
    ],
    weaknesses: [
      'Фиксированное рецептивное поле после обучения',
      'Менее гибок при смене сезонности',
      'Требует больше VRAM чем LSTM на тех же данных',
    ],
    bestFor: 'Длинные временные ряды (1000+ точек). Задачи где важна скорость.',
    notFor: 'Очень короткие ряды. Данные с нерегулярными временными интервалами.',
    assets: ['NASDAQ', 'S&P 500', 'NVIDIA', 'Tesla', 'Amazon'],
    refs: [
      {
  ...
  },
  {
    ...
    speedColor: 'text-amber-400',
    complexity: 'Высокая',
    tags: ['MLP', 'Разложение', 'Интерпретируемость'],
    description: `N-BEATS — архитектура от Oreshkin и соавторов (2019, Element AI). Полностью на MLP-блоках без рекуррентности и свёрток. Каждый блок предсказывает backcast (восстановление входа) и forecast (прогноз), а остаток передаётся следующему блоку.

Такое "разделяй и властвуй" позволяет каждому стеку специализироваться на разных компонентах ряда (тренд, сезонность). Победитель соревнований M4 и M5 по временным рядам.

В AlphaSignal: 2 стека по 3 блока, 256 нейронов в 4 полносвязных слоях, theta=64. Входной ряд плоский (нет 3D-тензора).`,
    strengths: [
      'Победитель M4 Competition по точности',
      'Интерпретируемое разложение на компоненты',
      'Нет рекуррентности — стабильное обучение',
      'Эффективен при небольшом количестве данных',
    ],
    weaknesses: [
      'Не умеет использовать дополнительные признаки',
      'Фиксированный горизонт прогноза',
      'Сложнее настраивать чем LSTM',
    ],
    bestFor: 'Стабильные финансовые ряды с трендом и умеренной волатильностью.',
    notFor: 'Данные с резкими структурными сдвигами. Криптовалюты.',
    assets: ['Dow Jones', 'Сбербанк', 'Лукойл', 'Роснефть', 'Chevron'],
    refs: [
      {
        authors: 'Oreshkin, B. N., et al.',
        year: 2020,
  ...
  },
  {
    ...
    icon: <IconStar size={22}/>,
    name: 'Transformer',
    full: 'Temporal Transformer (Vaswani et al., 2017)',
    color: '#2dd4bf',
    mape: '0.92%',
    speed: 'Медленно',
    speedColor: 'text-red-400',
    complexity: 'Высокая',
    tags: ['Внимание', 'SOTA', 'Параллельность'],
    description: `Трансформер — архитектура на основе механизма самовнимания (Self-Attention), предложенная Васвани и соавторами в 2017 году для NLP, а затем адаптированная для временных рядов.

Каждый токен (временная точка) "смотрит" на все остальные одновременно через Multi-Head Attention — это принципиально отличает трансформер от рекуррентных сетей, где информация передаётся последовательно.

В AlphaSignal: d_model=64, 4 головы внимания, 2 слоя энкодера, FFN=128, синусоидальное позиционное кодирование. Принимает скользящее окно 60 дней.`,
    strengths: [
      'Параллельная обработка всей последовательности',
      'Глобальный контекст — любая точка влияет на любую',
      'Сильнейший на длинных зависимостях',
      'Основа всех современных SOTA-моделей',
    ],
    weaknesses: [
      'Требователен к памяти: O(n²) по длине последовательности',
      'Обучается медленнее LSTM',
      'Переобучение на малых данных без регуляризации',
    ],
    bestFor: 'Долгосрочные прогнозы (недели, месяцы). Рынки с системными паттернами.',
    notFor: 'Малые датасеты (<500 точек). Очень короткий горизонт прогноза.',
    assets: ['S&P 500', 'NASDAQ', 'Apple', 'Google', 'ExxonMobil'],
    refs: [
      {
  ...
  },
  {
    ...
    speedColor: 'text-red-400',
    complexity: 'Очень высокая',
    tags: ['Google', 'Фьюжн', 'Интерпретируемость'],
    description: `TFT разработан командой Google Brain и опубликован в 2021 году. Комбинирует LSTM-энкодер для локального контекста, темпоральное самовнимание для долгосрочных зависимостей и GLU-ворота для отбора признаков.

GLU (Gated Linear Unit) — это механизм, аналогичный вентилям LSTM, но применённый к Feature Selection: gate(x) = sigmoid(W₁x) ⊙ linear(W₂x). Это позволяет модели автоматически "выключать" нерелевантные признаки.

Интерпретируем: по весам внимания можно понять какие прошлые периоды важны для прогноза.`,
    strengths: [
      'Интерпретируемость через attention weights',
      'GLU-ворота для автоматического отбора признаков',
      'Объединяет лучшее от LSTM и Transformer',
      'Устойчив к нерелевантным признакам',
    ],
    weaknesses: [
      'Самый сложный пайплайн среди одиночных моделей',
      'Медленное обучение',
      'Требует тщательной настройки гиперпараметров',
    ],
    bestFor: 'Задачи где важна интерпретируемость + точность: корпоративный риск-менеджмент.',
    notFor: 'Быстрый прототип. Малые данные.',
    assets: ['S&P 500', 'Сбербанк', 'Лукойл', 'Brent', 'Microsoft'],
    refs: [
      {
        authors: 'Lim, B., et al.',
        year: 2021,
        title: 'Temporal Fusion Transformers for Interpretable Multi-horizon Time Series Forecasting',
        venue: 'International Journal of Forecasting, 37(4)',
  ...
  },
  {
    ...
    speedColor: 'text-red-400',
    complexity: 'Очень высокая',
    tags: ['2023', 'Патчи', 'Лучший MAPE'],
    description: `PatchTST — архитектура 2023 года от Nie и соавторов (Princeton). Ключевая идея: разделить временной ряд на патчи (подпоследовательности) и обрабатывать их как токены — аналогично тому, как Vision Transformer обрабатывает участки изображения.

Каждый патч содержит локальный контекст (8 точек), что снижает длину последовательности для attention с N до N/patch_size. Это и экономит память, и заставляет модель обучаться на семантически значимых блоках, а не отдельных точках.

В AlphaSignal: окно 64, patch_size=8 → 8 патчей, d_model=64, 2 слоя MHA, GELU активации, learnable positional embedding.`,
    strengths: [
      'Лучший MAPE 0.76% — #1 среди всех одиночных моделей',
      'Патч-токенизация эффективнее чем point-wise внимание',
      'GELU-активации работают лучше ReLU на финансовых данных',
      'Learnable positional embedding вместо синусоидального',
    ],
    weaknesses: [
      'Длина окна должна делиться на patch_size',
      'Медленнее чем LSTM и TCN',
      'Требует много данных для полного раскрытия потенциала',
    ],
    bestFor: 'Среднесрочные прогнозы (1–3 месяца). Ряды с локальными паттернами внутри патча.',
    notFor: 'Очень короткие ряды. Внутридневные данные с нерегулярными паттернами.',
    assets: ['S&P 500', 'NASDAQ', 'NVIDIA', 'Tesla', 'Apple', 'Яндекс'],
    refs: [
      {
        authors: 'Nie, Y., et al.',
        year: 2023,
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      <div className="max-w-[1300px] mx-auto px-8 py-10">
        ...
        {/* Comparison bar */}
        <div className="grid grid-cols-8 gap-1.5 mb-8 p-3 bg-[var(--surface)] border border-[var(--border)] rounded-2xl">
          {MODELS.map(m => (
            <button key={m.id} onClick={() => setActive(m)}
              ...
              <span className="text-[9px] text-amber-400 font-mono">{m.mape}</span>
            </button>
          ))}
        </div>
